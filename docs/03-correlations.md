...
The partial correlation matrix can be computed as the Schur complement of $\Sigma$ when this is included in the block matrix $M$, defined as $M = \begin{bmatrix} \Sigma & B \\ B^T & C \end{bmatrix}$. Note that this simply correspods to the full correlations matrix taking into account all variables (variables of interest and confounders).

The conditional correlation matrix can be then computed as 

$$\Sigma \mid C = \Sigma - BC^{-1}B^T.$$ 
